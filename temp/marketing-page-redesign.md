...
> **Companion to:** `ux-and-positioning-strategy.md`
...
The current marketing page describes a research mission. It needs to describe a product position. Cathedral is the accountability layer for software development — the system of record that Jira, Slack, and Claude structurally cannot provide because they are edges, not the hub. The page must say why not just ChatGPT (capability is not accountability), why not just world models (prediction is not accountability; a world model is a brilliant architect with perfect memory and zero accountability), why not just Jira (tickets are not traceable requirements), what the buyer will actually see (a visual model that makes accountability state legible as graphs and diagrams), and who it is for (organizations already exposed to the cost of not having a substrate). The marketing pillars are: accountability not capability, the system of record your tools report into, see what your tools can't show you, derived not transcribed, and built for the cost of not having it. Every section reinforces one or more of these. The tagline changes. The feature pills become outcomes. The abstract becomes concrete. The Problem section names the RCT evidence (developers 19% slower with AI while believing they were faster — a feedback-loop problem the check engine solves). Every mention of "specification" carries the qualifier *machine-checkable*, because prose specs are still unstructured prose. And we speak honestly to the people who are about to get hurt, and let the hurt do the marketing.
...
- **UX & Positioning Strategy:** `_docs/ux-strategy/ux-and-positioning-strategy.md`
